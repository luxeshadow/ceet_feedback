...
export function validateRegister(user: Partial<User>) {
  ...
  if (!user.first_name || user.first_name.trim().length < 2) {
    throw new Error('Le prénom est obligatoire et doit contenir au moins 2 caractère');
  }
  ...
  if (!user.last_name || user.last_name.trim().length < 2) {
    throw new Error('Le nom est obligatoire et doit contenir au moins 2 caractère');
  }
