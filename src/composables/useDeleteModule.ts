...
export function useDeleteModule(
  ...
  const remove = async (id: number) => {
    ...
    try {
      ...
      showToast('Module supprimé avec succès !', { type: 'success' });
      return true;
    } catch (err: any) {
  ...
  const handleDelete = async (id: number, fetchModules: (page: number) => Promise<void>) => {
    if (confirmingIds.value.has(id)) {
      deletingIds.value.add(id);
      const success = await remove(id);
      if (success) {
        modules.value = modules.value.filter((mod: Module) => mod.id !== id);
        total.value -= 1;
        lastPage.value = Math.ceil(total.value / perPage.value);
        if (modules.value.length === 0 && currentPage.value > 1) {
          await fetchModules(currentPage.value - 1);
        }
      }
